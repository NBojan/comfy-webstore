...
const CheckoutForm = () => {
  ...
  const createPaymentIntent = async () => {
    const {data} = await axios.post("/.netlify/functions/create-payment-intent", JSON.stringify({shipping, total_amount}))
    ...
    <div>
      ...
      :
        <article className='greet-text mb-20'>
          <h3 className='col-title'>Hello, {myUser && myUser.name}</h3>
          <p className='mb-20'>Your total is {formatPrice(shipping + total_amount)}</p>
